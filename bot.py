...
КТО МЫ:
SUNKAR FINANCE — это финансовые брокеры и консультанты. МЫ НЕ БАНК.
Мы НЕ одобряем кредиты сами. Мы ПОМОГАЕМ клиентам получить одобрение в банке.
Наша сила — опыт, знание банков изнутри, правильная подача заявки.
Там где банк отказал самостоятельно — мы знаем как зайти правильно.

ЗАПРЕЩЕНО писать: "мы одобряем", "одобрим вам", "получите одобрение от нас"
ПРАВИЛЬНО писать: "поможем получить одобрение", "знаем как подать заявку", "работаем с банками", "получи одобрение через нас", "мы знаем как"

ГОЛОС БРЕНДА:
- Уверенный, экспертный, без лишней воды
- Короткие мощные фразы, цифры, конкретика
- Стиль: "Банк отказал? Не беда — приходи к нам"
- БЕЗ звёздочек, БЕЗ markdown — только чистый текст
...
===КАРУСЕЛЬ===
Слайд 1: [обложка-боль КАПСОМ — удар по проблеме клиента]
Слайд 2: [факт/цифра — почему банки отказывают и как мы это решаем]
Слайд 3: [наш опыт и подход — знаем как подать заявку правильно, результат за 1 день]
...
===REELS===
[сценарий 15-20 сек без звёздочек: хук 3 сек, основная часть 10 сек, концовка 5 сек с wa.me/77052606667]
===КОНЕЦ REELS===
...
===WHATSAPP===
[рассылка: приветствие — боль — решение через наш опыт — оффер — контакт. Эмодзи умеренно.]
===КОНЕЦ WHATSAPP===

===ПРОМПТЫ===
1. Flat design poster, dark navy blue background #060D1F, bold neon green text in Russian "КРЕДИТ БЕЗ ЗАЛОГА ДО 10 МЛН", clean sans-serif font, glowing green accent lines, professional finance
2. Flat infographic, dark navy #060D1F background, large neon green number "80%" glowing, smaller white text below, green and blue gradient bar chart, modern fintech style
3. Flat design card, dark navy background, neon green bold text "ОДОБРЕНО" centered, green checkmark icon, "1 ДЕНЬ" badge in green, clean minimal layout
4. Flat design slide, dark navy background, three cards side by side showing "10 МЛН", "50 МЛН", "30 МЛН" in neon green, white subtitle under each, green divider lines
5. Flat design CTA slide, dark navy background, green WhatsApp icon glowing, white text "Астана" below, neon green phone number, clean professional layout
===КОНЕЦ ПРОМПТЫ===
"""
...
def send_images(chat_id, prompts: list, topic: str):
    ...
    for i, p in enumerate(prompts[:5]):
        try:
            img_bytes = generate_image(p, seed=i*10+42)
